...
func (v *FileValidator) Validate(file *multipart.FileHeader) error {
	...
	if v.MaxSize > 0 && file.Size > v.MaxSize {
		return result.NewErrCodeMsg(100001, errors.New(fmt.Sprintf("文件大小超过限制，最大允许 %s", formatFileSize(v.MaxSize))).Error())
	}
	...
	if len(v.ForbiddenExts) > 0 {
		for _, forbidden := range v.ForbiddenExts {
			if ext == strings.ToLower(forbidden) {
				return result.NewErrCodeMsg(100001, errors.New(fmt.Sprintf("不允许上传 %s 类型的文件", ext)).Error())
			}
		}
	}
	...
	if len(v.AllowedExts) > 0 {
		...
		if !allowed {
			return result.NewErrCodeMsg(100001, errors.New(fmt.Sprintf("不支持的文件类型 %s，允许的类型：%s", ext, strings.Join(v.AllowedExts, ", "))).Error())
		}
	}
	...
	if len(v.AllowedMimes) > 0 {
		...
		if !allowed {
			return result.NewErrCodeMsg(100001, errors.New(fmt.Sprintf("不支持的文件 MIME 类型：%s", contentType)).Error())
		}
	}
...
func validateFilename(filename string) error {
	// 检查文件名长度
	if len(filename) > 255 {
		return result.NewErrCodeMsg(100001, errors.New("文件名过长，最大 255 个字符").Error())
	}
	...
	for _, char := range dangerousChars {
		if strings.Contains(filename, char) {
			return result.NewErrCodeMsg(100001, errors.New("文件名包含非法字符").Error())
		}
	}

	// 检查是否为空
	if strings.TrimSpace(filename) == "" {
		return result.NewErrCodeMsg(100001, errors.New("文件名不能为空").Error())
	}
